...
Every modification you make in the HTML or CSS file will be visible after reloading the page. 
...
"messageWelcome" and "objective" are variables that are in the server.py file. 
Local variables, here text and message, must be created within the render_template function and assigned variable values, 
here the variables messageWelcome and objective. 
These are the variables created locally in the render_template function, 
which will then be displayed in the index.html file using a syntax with double braces: {{ variable_to_display }}

The value in the double braces will be converted to HTML by Flask. 
You can change its style by pointing to the tag in which it is located.

Here there are 2 variables, but you can put as many variables as you want in the render_template parameters
